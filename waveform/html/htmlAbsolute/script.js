...
for (let i = 0; i < waveContainers.length; i++) {
  ...
  let baseline = containerSpecs.height - (containerSpecs.height * 0.50);

  let topPadding = parseFloat(
      window.getComputedStyle(container, null).getPropertyValue('padding-top'));
  let bottomPadding = parseFloat(window.getComputedStyle(container, null)
